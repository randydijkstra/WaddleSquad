  ...
  for tileNumber, tileId in pairs(layer.data) do
    ...
    if tileId > 0 then
      ...
      local tileIdLeftTop = iif( 
        tileNumber % layer.width == 0 or tileNumber < layer.width, 
        0, 
        layer.data[tileNumber - layer.width]
      )      
      local tileIdRightTop = iif( 
        tileNumber+2 % layer.width == 0 or tileNumber+2 < layer.width, 
        0, 
        layer.data[tileNumber+2 - layer.width]
      )
      local tileIdLeftBottom = iif( 
        tileNumber % layer.width == 0 or layer.data[tileNumber + layer.width] == nil, 
        0, 
        layer.data[tileNumber + layer.width]
      )
      ...
      --[[ 
      prioritize left tiles
      current broken 
      if tileIdLeft > 0 then
        belongsTo = tileNumber - 1 
      elseif tileIdTop > 0 and tileIdRightTop > 0 and tileIdLeftTop <=0 then
        belongsTo =  tileNumber - layer.width
      end
      --]]
